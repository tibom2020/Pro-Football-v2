...
export const BetHistory: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Summary Section */}
      <div className="bg-white dark:bg-slate-900 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-slate-800 transition-colors">
        ...
        <div className="flex justify-around text-center">
            <div>
                <div className="text-xs text-gray-500 dark:text-gray-400">Tổng cược</div>
                <div className="text-lg font-bold text-gray-800 dark:text-white">{summary.totalStake.toLocaleString()}</div>
            </div>
            <div>
                <div className="text-xs text-gray-500 dark:text-gray-400">Lãi/Lỗ</div>
                <div className={`text-lg font-bold ${summary.totalPL >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
      ...
      {filteredTickets.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredTickets.map(ticket => {
             ...
             const ticketDate = new Date(ticket.createdAt || parseInt(ticket.id));
             return (
                <div key={ticket.id} className="bg-white dark:bg-slate-900 rounded-xl p-3 shadow-sm border border-gray-100 dark:border-slate-800 transition-colors">
                    <div className="flex justify-between items-start">
                        <div>
                            <div className="font-semibold text-xs text-gray-500 dark:text-gray-400 truncate max-w-[200px]">{ticket.matchName}</div>
                            <div className="font-bold text-gray-800 dark:text-white">{ticket.betType} {ticket.handicap} @{ticket.odds.toFixed(2)}</div>
                     ...
                     <div className="flex justify-between items-end mt-2 pt-2 border-t border-gray-100 dark:border-slate-800">
                        ...
                        <div className="text-right">
                            <div className="text-sm dark:text-gray-300">Cược: <span className="font-semibold">{ticket.stake.toLocaleString()}</span></div>
                            {ticket.status === 'won' && (
                                <div className="text-xs font-bold text-green-600 dark:text-green-400">Lãi: +{(ticket.stake * ticket.odds - ticket.stake).toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
                            )}
                            {ticket.status === 'lost' && (
                                <div className="text-xs font-bold text-red-600 dark:text-red-400">Lỗ: -{ticket.stake.toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
                            )}
                            {ticket.status === 'push' && (
                                <div className="text-xs font-bold text-gray-600 dark:text-gray-400">Hoàn tiền</div>
                            )}
                            {ticket.status === 'won_half' && (
                                <div className="text-xs font-bold text-green-600 dark:text-green-400">Lãi: +{((ticket.stake * ticket.odds - ticket.stake) / 2).toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
                            )}
                             {ticket.status === 'lost_half' && (
                                <div className="text-xs font-bold text-red-600 dark:text-red-400">Lỗ: -{(ticket.stake / 2).toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
                            )}
                        </div>
                    </div>
                    {/* Action Buttons for Pending Tickets */}
                    {ticket.status === 'pending' && (
                        <div className="flex gap-2 items-center flex-wrap justify-end mt-2 pt-2 border-t border-dashed border-gray-300 dark:border-slate-700">
