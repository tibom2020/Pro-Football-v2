...
export const TicketManager: React.FC<TicketManagerProps> = ({ match, latestOverOdds, latestHomeOdds, latestH1HomeOdds, latestH1OverOdds }) => {
  ...
  const [showChecklist, setShowChecklist] = useState(false); // State to control checklist modal
  ...
  const currentHandicap = useMemo(() => {
    ...
    if (betType.includes('Đội khách')) {
      ...
      if (invertedNum > 0) return `+${formatted}`;
      return formatted;
    }
    return undefined; 
  }, [betType, latestOverOdds, latestHomeOdds, latestH1HomeOdds, latestH1OverOdds]);

  // Initial handler - just validation and opening the checklist
  const handleInitiateAddTicket = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    // Open the checklist instead of saving directly
    setShowChecklist(true);
  };

  // Final handler - called after checklist is confirmed
  const confirmAddTicket = () => {
    ...
    // Haptic Feedback for Mobile (Success vibration)
